...
import { MerkleTreeProof, prepareDataForMultiConditions, writeExpData } from "./util/data";
...
async function evaluateASetting(treeHeight: number, numConditions: number, mode: zkVCMode) {
    ...
    await writeExpData(expData, "offchain", `offchain-${treeHeight}-${numConditions}-${mode}-${options.runningClient}`);
}
